...
void information_game(GameListNode* pnewHead,int situation) {//输入头节点和对应的位置(第几个)
	...
	if (sit == 0)//报名,修改进行中界面
	{
		...
		case 2: game_socre_print(game); system("pause"); information_game(pnewHead, situation); break;//返回
		...
		switch (x) {
		case 1: game_socre_print(game); system("pause"); information_game(pnewHead, situation); break;//成绩查询
		case 2: break;//返回
		}
	}
}
...
void information_player(PlayerListNode* pnewHead, int situation) {
	...
	for (int j = 0; j < player.game_number; j++) {
		...
		if (sit > 0) {
			printf("项目成绩:%0.2f ", player.score[j].score);//项目成绩
